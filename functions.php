...
function checkLoggedIn () {
	...
	if ( isset($_SESSION['username']) || isset($_COOKIE['client-ID'])) {
...
function fetchProjectData($projectID) {
		 ...
		 try {
			...
			$dbProjectQuery = 'SELECT *, DATE_FORMAT(_added_PROJECT, "%e %b %Y") AS added_PROJECT, DATE_FORMAT(date_filming_PROJECT, "%e %b %Y") AS filming_PROJECT ,DATE_FORMAT(date_firstDelivery_PROJECT, "%e %b %Y") AS firstDelivery_PROJECT, DATE_FORMAT(date_approval_PROJECT, "%e %b %Y") AS approval_PROJECT, DATE_FORMAT(date_finalDelivery_PROJECT, "%e %b %Y") AS finalDelivery_PROJECT, DATE_FORMAT(_completed_brief_PROJECT, "%e %b %Y") AS completed_brief_PROJECT FROM project LEFT JOIN client ON project._kf_client_PROJECT = client._kp_CLIENT WHERE _kp_PROJECT = '.$projectID.' AND live_PROJECT IS NOT NULL';
...
function fetchActiveProjectData($clientID) {
		 ...
		 try {
			...
			$dbProjectQuery = 'SELECT SQL_CALC_FOUND_ROWS *, DATE_FORMAT(_added_PROJECT, "%e %b %Y") AS added_PROJECT, DATE_FORMAT(date_firstDelivery_PROJECT, "%e %b %Y") AS firstDelivery_PROJECT, DATE_FORMAT(date_approval_PROJECT, "%e %b %Y") AS approval_PROJECT, DATE_FORMAT(date_finalDelivery_PROJECT, "%e %b %Y") AS finalDelivery_PROJECT FROM project LEFT JOIN client ON project._kf_client_PROJECT = client._kp_CLIENT LEFT JOIN video ON project._kp_PROJECT = video._kf_project_VIDEO WHERE _kf_client_PROJECT = '.$clientID.' AND live_PROJECT IS NOT NULL AND status_PROJECT = "Active" GROUP BY project._kp_PROJECT ORDER BY video._added_VIDEO DESC';
...
function getVimeoThumb($vimeoID) {
	...
	$xml = new SimpleXMLElement(file_get_contents("http://vimeo.com/api/oembed.xml?url=http%3A//vimeo.com/".$vimeoID)); 
